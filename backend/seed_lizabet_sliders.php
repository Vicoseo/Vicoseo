...
/**
 * Lizabet Sites — Slider Promotion Seed
 * Adds display_type='slider' promotions WITHOUT touching existing card promotions.
 * 4 sites × 8 sliders each = 32 total
 *
 * Usage:
 *   cd /var/www/multi-tenant-cms/backend
 *   php artisan tinker --execute="require 'seed_lizabet_sliders.php';"
 */
...
// Each site gets 8 slider promotions (same sets as their card promos)
$siteSets = [
    'girislizabet.site' => [
...
foreach ($siteSets as $domain => $slugs) {
    ...
    foreach ($slugs as $order => $slug) {
        ...
        echo "  [{$order}] SLIDER: {$title}\n";
    }

    echo "  -> {$domain}: " . count($slugs) . " sliders inserted\n\n";
}

echo "Done! All Lizabet slider promotions seeded.\n";
echo "Existing card promotions were NOT modified.\n";
